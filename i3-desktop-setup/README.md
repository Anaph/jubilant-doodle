...
- строка состояния на **i3blocks** (светлый текст на жёстко-тёмном баре): заряд
  батареи (AXP228), **потребление в ваттах** (V×I из sysfs), уровень 4G-сигнала
  донгла, CPU-график, диск, память, время; состояние сети — в трее (nm-applet);
...
- **энергосбережение** (подсветка — главный потребитель):
  - **кнопка питания → глубокий «сон» с блокировкой** (не suspend — CM5 его не
    умеет надёжно; система не замораживается, поэтому **просыпается мгновенно**):
    короткое нажатие → **i3lock** + подсветка off + CPU `powersave` + Wi-Fi/сеть off
    + (AIO) USB-рейл off; повторное нажатие будит (восстанавливает подсветку/частоту/
    сеть; экран остаётся заблокирован — вводишь пароль). Кнопку обрабатывает демон
  ...
  - **снижение частоты опроса USB-HID** в `cmdline.txt` (`usbhid.mousepoll=16
    usbhid.kbpoll=32`, бэкап `*.i3ds.bak`, **после перезагрузки**): хост опрашивает
    встроенные клавиатуру/трекбол ~125 Гц каждый (≈200 прерываний/с — те самые
...
> `video`/`input` подхватываются на старте). В «сне» рвётся Wi-Fi/4G (так
